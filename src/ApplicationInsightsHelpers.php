<?php
namespace Fredylg\AzureApplicationInsights;
...
class ApplicationInsightsHelpers
{

    /**
     * @var ApplicationInsightsServer
     */
    private $appApplicationInsights;


    public function __construct(ApplicationInsightsServer $appApplicationInsights)
    {
        $this->appApplicationInsights = $appApplicationInsights;
    }
    ...
    public function trackPageViewDuration($request)
    {
        if ($this->telemetryEnabled()) {
            if (session()->has('ms_application_insights_page_info')) {
                $this->appApplicationInsights->telemetryClient->trackMessage('browse_duration',
                    $this->getPageViewProperties($request));
                try {
                    $this->appApplicationInsights->telemetryClient->flush();
    ...
    public function trackRequest($request, $response)
    {
        if ($this->telemetryEnabled())
        {
            if ($this->appApplicationInsights->telemetryClient)
            {
                $this->appApplicationInsights->telemetryClient->trackRequest(
                    'application',
                    $request->fullUrl(),
    ...
    {
        if ($this->telemetryEnabled()) {
            $this->appApplicationInsights->telemetryClient->trackException($e,
    ...
    public function flashPageInfo($request)
    {
        if ($this->telemetryEnabled())
        {
            session()->flash('ms_application_insights_page_info', [
                'url' => $request->fullUrl(),
    ...
    {
        return isset($this->appApplicationInsights->telemetryClient);
    }
